...
// 
// Description:
// This module contains the code to handle Tapes under DriveMonitor
// 

// Update Log:
//	$Log: /Gemini/Odyssey/IsmDriveMonitor/DmTapes.cpp $
// 
...
/*************************************************************************/
// DM_Do_Tape
// Inquiry found a Sequential Storage type device, start the statemachine
// for tapes.
/*************************************************************************/
void DriveMonitorIsm::DM_Do_Tape(DM_CONTEXT * pDmc)
{
	DM_DEVICE_STATE	*pDMState = pDmc->pDMState;
	DeviceDescriptor	*pDevice;

	TRACE_ENTRY(DM_Do_Tape);
	...
	// copy the inquiry data to the Device Descriptor Table
...
/*************************************************************************/
// DM_Do_Update_Tape_PTS
// Update the DeviceDescriptor with the tape device data from the last
// device scanned.  Check if an update is allowed before the update,
// skip if not.
/*************************************************************************/
void DriveMonitorIsm::DM_Do_Update_Tape_PTS(DM_CONTEXT *pDmc)
{
	STATUS			 status;
	DM_DEVICE_STATE	*pDMState = pDmc->pDMState;
	
	DM_TRACE_ENTRY(DM_Do_Update_Tape_PTS);

#if defined(DM_DEBUG) && defined(_DEBUG)
		char			s[128];

		TRACEF(TRACE_L2, ("\n\rFound Tape, Id=%d, LUN=%d", 
							config.xlate[pDmc->drive_number],
							pDmc->lun_number));
	...
	{
		DmFindDescriptor(pDmc, (pDMCallback_t)&DM_Do_Update_Tape_PTS_1);
		return;
	}
	...
	if (DM_Check_PTS_Update_OK() == 0)
	{
		...
		DM_Do_Update_Tape_PTS_1(pDmc);
		return;
	}
	
	// Update the row this descriptor goes to
	status = DmTableUpdateDeviceDesc(pDmc, (pDMCallback_t)&DM_Do_Update_Tape_PTS_1);
	
} // DM_Do_Update_Tape_PTS


/*************************************************************************/
// DM_Do_Update_Tape_PTS_1
// Update the PathDescriptor with the device data from the last drive
// scanned.
/*************************************************************************/
void DriveMonitorIsm::DM_Do_Update_Tape_PTS_1(DM_CONTEXT *pDmc)
{
	STATUS			status;
	
	DM_TRACE_ENTRY(DM_Do_Update_Tape_PTS_1);
	...
	status = DmTableUpdatePD(pDmc, (pDMCallback_t)&DM_Do_Update_Tape_PTS_2);
	
} // DM_Do_Update_Tape_PTS_1


/*************************************************************************/
// DM_Do_Update_Tape_PTS_2
// Update the StorageRollCall with the device data from the last drive
// scanned.
/*************************************************************************/
void DriveMonitorIsm::DM_Do_Update_Tape_PTS_2(DM_CONTEXT *pDmc)
{
	STATUS			status;
	
	DM_TRACE_ENTRY(DM_Do_Update_Tape_PTS_2);
